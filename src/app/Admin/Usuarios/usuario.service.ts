...
// Método para verificar si el usuario está autenticado
  isLoggedIn(): boolean {
    ...
    const expirationDate = payload.exp * 1000; // La fecha de expiración está en segundos, por eso multiplicamos por 1000
    const now = new Date().getTime();

    if (expirationDate < now) {
      console.log('El token ha expirado.');
      this.logout(); // Cerrar sesión si el token ha expirado
      return false;
    }
